with ranked as (
  select
    id,
    row_number() over (
      partition by user_id, origin_generation_id
      order by updated_at desc nulls last, id desc
    ) as rn
  from public.flows
  where origin_type = 'share_import'
...
create unique index if not exists flows_share_import_origin_generation_uniq
  on public.flows (user_id, origin_generation_id)
  where origin_type = 'share_import'
    and origin_generation_id is not null;
