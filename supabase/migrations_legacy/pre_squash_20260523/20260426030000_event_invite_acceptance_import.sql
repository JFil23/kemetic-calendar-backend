create or replace function public.event_share_import_client_event_id(
  p_share_id uuid
)
returns text
language sql
...
create or replace function public.sync_event_share_import_marker()
returns trigger
language plpgsql
security definer
...
drop trigger if exists trg_event_shares_sync_import_marker
on public.event_shares;

create trigger trg_event_shares_sync_import_marker
before insert or update on public.event_shares
for each row
execute function public.sync_event_share_import_marker();

drop trigger if exists trg_event_shares_sync_calendar_copy
on public.event_shares;

create trigger trg_event_shares_sync_calendar_copy
after insert or update or delete on public.event_shares
for each row
execute function public.sync_event_share_calendar_copy();

-- Backfill existing rows so already-accepted invites immediately appear in the
-- recipient's calendar, and stale auto-imported copies are removed.
