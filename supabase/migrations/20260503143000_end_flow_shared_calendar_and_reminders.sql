...
begin
  ...
  retired_reminders as (
    -- Keep the legacy reminders ledger from outliving the flow lifecycle.
...
'Canonical end-flow lifecycle RPC. Owners and accepted shared-calendar owners/editors may end a flow; the RPC ends it, prunes future materialized rows from one cutoff, retires scheduled notifications, retires reliably linked reminders, and removes completions for deleted events in a single transaction.';
