...
-- Preserve every accounting predicate while allowing PostgreSQL to inline
-- single-use stages. Keep materialization only where a stage is reused or
-- where unresolved-action ownership precedence requires one stable result.
...
'Canonical set-based flow activity summary. Reused ownership stages are materialized; single-use event classification and aggregation stages remain inlineable.';
