...
-- candidate_flows already admits only rows for which the canonical deleted-
-- state predicate is false. Do not carry notes into every event row and parse
-- them again during aggregation. All ownership and accounting branches remain
-- otherwise unchanged.
...
'Canonical set-based flow activity summary. Candidate deletion state is resolved once; selected direct events and legacy ownership branches retain their existing indexed semantics.';
