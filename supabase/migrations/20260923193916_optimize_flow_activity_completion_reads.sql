...
-- Legacy events without flow_local_id are rare, but the former fallback
-- compared every unresolved action with every candidate flow and reparsed a
-- flow's metadata for each comparison. Build that action map once per flow.
...
begin
  ...
  flow_action_map as materialized (
    select
      ...
      action_id
    from candidate_flows cf
    join public.flows source_flow
      on source_flow.id = cf.id
    cross join lateral unnest(
      public.flow_action_ids_from_metadata(source_flow.ai_metadata)
    ) action_id
    where exists (select 1 from unresolved_actions)
  ),
...
'Canonical set-based flow activity summary. Legacy action ownership is expanded once per candidate flow, and completion keys are read through the selected flow set.';
