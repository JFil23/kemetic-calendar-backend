...
begin
  ...
  select
    ...
    summary.remaining_live_event_count as remaining_event_count,
    summary.is_counted_active
  from private.flow_activity_summary_v1(v_uid, p_flow_ids) summary;
end;
$$;
...
create or replace function public.get_currently_active_imported_flows_v1(
  p_share_ids uuid[]
)
returns table (
  id bigint,
...
begin
  ...
  if coalesce(cardinality(p_share_ids), 0) = 0 then
    return;
  end if;
  ...
  select
    ...
    f.origin_share_id,
    f.created_at
  from private.flow_activity_summary_v1(v_uid, null) summary
  join public.flows f
    on f.id = summary.flow_id
   and f.user_id = v_uid
  where summary.is_counted_active
    and (
      f.share_id = any(p_share_ids)
      or f.origin_share_id = any(p_share_ids)
    )
  order by f.created_at desc;
end;
$$;
...
revoke all on function public.get_currently_active_imported_flows_v1(uuid[])
  from public;
revoke all on function public.get_currently_active_imported_flows_v1(uuid[])
  from anon;
grant execute on function
  public.get_currently_active_imported_flows_v1(uuid[])
  to authenticated;

comment on function private.flow_activity_summary_v1(uuid, bigint[]) is
'Set-based flow filing accountant shared by bounded public consumers. It reads base event, completion, and tombstone tables without expanding user_event_filing_items_client.';

comment on function public.get_my_filed_flows_v1(integer) is
'Authenticated My Flows filing rows backed by the set-based flow accountant rather than flow_filing_items_client.';

comment on function public.get_currently_active_imported_flows_v1(uuid[]) is
'Authenticated current import-state lookup backed by the set-based flow accountant.';

comment on function public.get_profile_flow_counts(uuid) is
'Profile flow counts backed by the set-based flow accountant.';
