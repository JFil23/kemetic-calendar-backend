create or replace function public.user_event_has_suppressing_tombstone(
  p_user_id uuid,
...
begin
  ...
  if lower(v_client_event_id) not like 'reminder:%' then
    return false;
  end if;
...
$$;
revoke all on function public.user_event_has_suppressing_tombstone(uuid, text) from public;
grant execute on function public.user_event_has_suppressing_tombstone(uuid, text) to authenticated;
grant execute on function public.user_event_has_suppressing_tombstone(uuid, text) to service_role;
comment on function public.user_event_has_suppressing_tombstone(uuid, text) is
'Fast security-definer tombstone check for aggregate filing counts. It suppresses exact and reminder-series tombstones without running reminder orphan reconciliation.';
...
calendar_event_counts as (
  select
    ...
    count(distinct er.filed_flow_id) filter (
      where not er.is_deleted
        and er.filed_flow_id is not null
        and er.date_lifecycle = 'active'
    ) as live_flow_count
  from event_rows er
...
'Client-safe flow-level filing view. Flow event counts use indexed flow_local_id rows first and a fast security-definer tombstone check for aggregate startup paths.';
comment on view public.shared_calendar_filing_items_client is
'Client-safe filing view for accepted calendars. Calendar event counts are accepted-calendar scoped and use a fast security-definer tombstone check for aggregate startup paths.';
