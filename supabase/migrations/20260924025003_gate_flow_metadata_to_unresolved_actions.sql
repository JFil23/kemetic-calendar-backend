...
-- Keep the canonical accountant and every public consumer unchanged. Resolve
-- direct and explicit legacy ownership from the one user-bounded event read,
-- then inspect flow metadata only when an event still has action-only
-- ownership. This restores the proven unresolved-action gate without adding a
-- second read authority or changing ownership precedence.
...
begin
  ...
  unresolved_actions as materialized (
    select distinct btrim(uer.action_id) as action_id
    from user_event_rows uer
    where uer.flow_local_id is null
      and uer.referenced_flow_id is null
      and nullif(btrim(coalesce(uer.action_id, '')), '') is not null
  ),
...
'Canonical set-based flow activity summary. User events and completions are each read once by user; metadata action ownership is evaluated only for unresolved action-only fallback events.';
