-- Helper selection for flow outcome cron (service_role only)
...
'Returns flow_ids for ended, non-hidden, non-reminder flows with no outcome row for the start_date window; used by cron to call compute_flow_outcome';
