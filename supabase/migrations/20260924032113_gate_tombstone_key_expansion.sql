...
-- Preserve every deletion predicate while skipping reminder/key expansion
-- when the user has no active suppressing tombstones. The active tombstone set
-- remains user-bounded and uses the existing partial index.
...
'Canonical set-based flow activity summary. Tombstone and reminder key expansion occurs only when the user has active suppressing tombstones.';
