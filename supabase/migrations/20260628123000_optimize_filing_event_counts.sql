...
event_rows as (
  select
    ...
    (
      ...
      or public.flow_is_deleted_state(fs.active, fs.is_hidden, fs.notes)
    ) as is_deleted,
...
'Client-safe flow-level filing view. Flow event counts are derived from authenticated user-scoped base event rows after candidate flows are known.';
comment on view public.shared_calendar_filing_items_client is
'Client-safe filing view for accepted calendars. Calendar event counts are derived from accepted-calendar-scoped base event rows after candidate calendars are known.';
