...
comment on function public.get_commons_home(date, text, text, integer) is
  'Canonical bounded Commons home: privacy-safe rhythm labels, answers, owned/member Reading Houses, public rooms, fragments, and complete social snapshots from get_profile_feed.';

-- Keep the former RPC name as a compatibility delegate while older RC
-- clients age out. It has no independent query or payload contract.
...
comment on function public.get_commons_home_cards(date, text, text, integer) is
  'Compatibility delegate to the canonical get_commons_home RPC; contains no independent Commons or social-feed implementation.';
