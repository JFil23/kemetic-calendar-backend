...
-- Restore the exact pre-regression flow accountant. The September 23
-- rewrites did not improve the real account path and are no longer active.
...
-- Preserve the current appearance column while restoring the filing function
-- used by the last known-good RC.
...
comment on function public.get_my_filed_flows_v1(integer) is
  'Pre-regression My Flows filing authority with appearance included in the existing row contract.';
