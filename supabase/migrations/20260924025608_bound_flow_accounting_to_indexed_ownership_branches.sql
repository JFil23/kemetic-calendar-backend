...
-- Preserve the canonical accountant and its ownership precedence while using
-- the existing mutually exclusive ownership indexes. Direct events are read
-- only for selected flows; explicit legacy references use the reference
-- index; action-only legacy rows use the action index and enter metadata
-- matching only after unresolved actions exist.
...
'Canonical set-based flow activity summary. Selected direct events and the two legacy fallback ownership branches use their existing indexes; metadata matching is gated by unresolved action-only rows.';
