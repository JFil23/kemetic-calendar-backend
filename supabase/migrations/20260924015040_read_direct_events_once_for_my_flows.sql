...
-- its two unbounded joins. Direct events are read once through the selected
-- flow-id set, a fallback event is resolved once from its stored references,
-- and flow action ids are expanded once per candidate flow.
...
'Canonical set-based flow activity summary. Direct events are read once through the selected flow-id set, legacy event ownership is resolved once, flow action ids are expanded once per candidate flow, and completion keys are read through the selected flow set.';
